...
function removeFirstOccurrences(str, value) {
  const iStart = str.indexOf(value);
  const iEnd = iStart + value.length;
  return iStart > -1 ? str.slice(0, iStart) + str.slice(iEnd) : str;
}
...
function countVowels(str) {
  const vowels = /[a, e, i, o, u, y, A, E, I, O, U, Y]/g;
  const arr = str.match(vowels);
  return arr === null ? 0 : str.match(vowels).length;
}
...
function isPalindrome(str) {
  const reverseStr = str.toLowerCase().match(/[a-z]/g).reverse().join('');
  return str.toLowerCase().match(/[a-z]/g).join('') === reverseStr;
}
...
function reverseWords(str) {
  ...
  const arr1 = [];
  arr.forEach((el) => {
    const wordRev = el.split('').reverse().join('');
    arr1.push(wordRev);
  });
  return arr1.join(' ');
}
...
function encodeToRot13(str) {
  ...
  const n = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str
    .split('')
    .map((el) => (a.includes(el) ? n[a.indexOf(el)] : el))
    .join('');
}
